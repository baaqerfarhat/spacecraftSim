...
def spawn_blender_procgen_assets(
    ...
    # Skip generation if all the files already exist and they are too recent
    skip_generation = True
    for current_seed in range(cfg.seed, cfg.seed + cfg.num_assets):
        filepath = path.join(outdir, f"{cfg.name}{current_seed}{cfg.ext}")
        if path.exists(filepath) and (
            cfg.overwrite_min_age < 0
            or (cfg.overwrite_min_age > time.time() - path.getmtime(filepath))
        ):
            continue
        else:
    ...
    if skip_generation:
        print(
            f"[INFO] {cfg.num_assets} procedural '{cfg.name}' asset(s) for '{prim_path}' are already cached. Skipping generation."
        )
    else:
